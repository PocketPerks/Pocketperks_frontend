          ...
          >
            {!bubble.isYou && (
              <div className="w-8 h-8 rounded-full overflow-hidden flex-shrink-0 mt-2 border border-gray-200">
                <Image
                  src={bubble.avatar || '/logos/pfp.jpg'} // ✅ fallback diya
...
export default function ChatPage() {
  ...
  useEffect(() => {
    const newSocket: Socket = io('http://localhost:5000', {
      transports: ['websocket'], // ✅ kabhi kabhi polling error aata hai
    });
    setSocket(newSocket);

    newSocket.on('contacts', (list: Contact[]) => {
      setContacts(list);
    ...
    newSocket.on('messages', ({ contactId, messages }: { contactId: number; messages: Message[] }) => {
      setConversations((prev) => ({ ...prev, [contactId]: messages }));
    });
  ...
  const handleSend = () => {
    if (!input.trim() || selectedContact === null || !socket) return;
    ...
    socket.emit('sendMessage', { contactId: selectedContact, message: input });
    setInput('');
  };

  const filteredContacts = contacts.filter((c) => c.tab === activeTab);
  const messages = selectedContact ? conversations[selectedContact] || [] : [];
          ...
          {filteredContacts.map((contact) => (
            ...
            >
              ...
              <div className="w-12 h-12 rounded-full bg-gray-300 border-2 border-white overflow-hidden flex-shrink-0 relative">
                <Image
                  src={contact.avatar || '/logos/pfp.jpg'} // ✅ fallback diya
