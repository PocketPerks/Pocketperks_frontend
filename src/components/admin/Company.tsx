...
import Modal from "../ui/Modal"; // your framer-motion modal
...
export default function Company() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setCompanies((prev) => [
      ...prev,
      { ...form, status: "Active" } as CompanyType,
    ]);
    setForm({ categories: [] });
    setOpen(false);
  };
            ...
            {(form.categories || []).map((cat, index) => (
              <div key={index} className="grid grid-cols-2 gap-4 mb-2">
                ...
                <input
                  ...
                  value={cat.rate}
                  onChange={(e) =>
                    handleCategoryChange(index, "rate", e.target.value)
                  }
                  className="p-2 border border-gray-200 rounded-xl placeholder-gray-400 text-black"
                />
              </div>
